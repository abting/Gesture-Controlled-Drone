...
with tf.name_scope("ModelV2") as scope:
    model = input_img
    model = nb.attach_conv_layer(model, 32, summary=False)
    model = nb.attach_relu_layer(model)
    model = nb.attach_conv_layer(model, 32, summary=False)
    model = nb.attach_relu_layer(model)
    model = nb.attach_pooling_layer(model)

    model = nb.attach_conv_layer(model, 64, summary=False)
    model = nb.attach_relu_layer(model)
    model = nb.attach_conv_layer(model, 64, summary=False)
    model = nb.attach_relu_layer(model)
    model = nb.attach_pooling_layer(model)

    model = nb.attach_conv_layer(model, 128, summary=False)
    model = nb.attach_relu_layer(model)
    model = nb.attach_conv_layer(model, 128, summary=False)
    model = nb.attach_relu_layer(model)
    model = nb.attach_pooling_layer(model)
    ...
    model = nb.attach_dense_layer(model, 200, summary=False)
    model = nb.attach_sigmoid_layer(model)
    model = nb.attach_dense_layer(model, 32, summary=False)
    model = nb.attach_sigmoid_layer(model)
...
dg = DataSetGenerator("./Gesture_Images/image_dataSet")
...
with tf.Session() as sess:
    ...
    filename = "./summary_log/run" + datetime.datetime.now().strftime("%Y")
